...
ggplot(loo_df, aes(x = model, y = dloo, color = dloo==0)) +
  ...
  labs(
    x = "",
    y = expression(Delta*ELPD~"(relative to best model)")
  )+
...
Based on ELPD statistics (@mod_compare) we selected model 7 as the best from those explored. This model included year-to-year variation in spawner abundance, arrival timing, arrival spread, and death spread, but assumed that residence time and observation error is constant across years. Model 8, which also allowed residence time to vary across years had an equivalent ELPD ($\Delta$ELPD = `r round(loo_df$dloo[2], 1)`, SE = `r round(loo_df$se[2], 1)`). We have selected the simpler model, for parsemony, but contrast the results from both where appropriate. 

MORE!

## Spawning timing

The average arrival day ($\text{arrival}_{\mu}$) across years was estimated to be `r round(median(post$arrival_mu)+day_stand, 1)` (posterior median; 95% CI: `r round(quantile(post$arrival_mu, 0.025)+day_stand, 1)`--`r round(quantile(post$arrival_mu, 0.975)+day_stand, 1)`) which corresponds approximately to `r format(ymd("2025-01-01") + round(median(post$arrival_mu)+day_stand - 1), "%B %e")`. Interannual variation in arrival date ($\text{arrival}_{\sigma}$) was estimated as a standard deviation of `r round(median(post$arrival_sigma), 1)` days. As a result, the median estimated arrival date ranged from `r format(ymd("2025-01-01") + round(min(apply(post$arrival, 2, median))+day_stand - 1), "%B %e")` to `r format(ymd("2025-01-01") + round(max(apply(post$arrival, 2, median))+day_stand - 1), "%B %e")` (@fig-timings A).

The average arrival spread ($e^{\text{arrival_spread}_{\mu}}$) across years was estimated to be `r round(median(exp(post$arrival_spread_mu)), 1)` days (posterior median; 95% CI: `r round(quantile(exp(post$arrival_spread_mu), 0.025), 1)`--`r round(quantile(exp(post$arrival_spread_mu), 0.975), 1)`). Arrival spread varied across years by a multiplicative factor ($e^{\text{arrival_spread}_{\sigma}}$) of `r round(median(exp(post$arrival_spread_sigma)), 1)` As a result, the median estimated arrival spread ranged from `r round(min(apply(post$arrival_spread, 2, median)) - 1)` to `r round(min(apply(post$arrival_spread, 2, max)) - 1)` days (@fig-timings B).

The average death spread ($e^{\text{death_spread}_{\mu}}$) across years was estimated to be `r round(median(exp(post$death_spread_mu)), 1)` days (posterior median; 95% CI: `r round(quantile(exp(post$death_spread_mu), 0.025), 1)`--`r round(quantile(exp(post$death_spread_mu), 0.975), 1)`). Death spread varied across years by a multiplicative factor ($e^{\text{death_spread}_{\sigma}}$) of `r round(median(exp(post$death_spread_sigma)), 1)` As a result, the median estimated death spread ranged from `r round(min(apply(post$death_spread, 2, median)) - 1)` to `r round(min(apply(post$death_spread, 2, max)) - 1)` days (@fig-timings C).


```{r fig-timings, echo = FALSE, warning=FALSE, message=FALSE, fig.cap="Posterior distributions of timing, entry_spread, and exit_spread. Points and error bars show the posterior median and the 66% and 95% credible intervals.", fig.align='center', fig.height=8, fig.width=6}
...
Residence was estimated as `r round(median(post$residence), 1)` days (95% CI; `r round(quantile(post$residence, 0.025), 1)`--`r round(quantile(post$residence, 0.975), 1)`). 
...
All years as ggplot facets (@fig-spawnercurves). free y-axis, please. Is it useful to repeat with y-axis as logs? Otherwise it looks like high counts are badly fit, but that is because residual error is assumed similar to sampling error which is proportional (multiplicative, % accuracy). <!--I tried it with free y axis, but that really emphasizes the deviations from the predicted for the high count values. This does the dual job of showing the fit and the year to year variability in total abundance, linking it to the abundance time series figure. I don't think the log fit is necessary. But, if you have strong feelings, I am happy to change.-->
