...
Surveys of live spawners have been conducted in the main sockeye spawning area---also known as the index section---of the Okanagan river between Osoyoos and Vasseau lakes (@fig-map) since `r min(index_sk$year)`. All together, surveys have been conducted on `r nrow(index_sk)` days, ranging between `r min(table(index_sk$year))` and `r max(table(index_sk$year))` surveys per year. All surveys occurred between `r format(min(update(index_sk$date, year = 2000)), "%B %e")` and `r format(max(update(index_sk$date, year = 2000)), "%B %e")`.
...
## Model selection 
Based on ELPD statistics (@mod_compare B) we selected model 7 as the best from those explored. This model included year-to-year variation in spawner abundance, arrival timing, arrival spread, and death spread, but assumed that residence time and observation error is constant across years. Model 8, which also allowed residence time to vary across years had an equivalent ELPD ($\Delta$ELPD = `r round(loo_df$dloo[2], 1)`, SE = `r round(loo_df$se[2], 1)`). We have selected the simpler model, for parsimony, but contrast the results from both where appropriate.  

The effective number of parameters ($\mathrm{p}_{\mathrm{loo}}$) increased when arrival timing variation was introduced (model 1 vs. 2), and also when any of arrival spread, death spread, or residence were allowed to vary (models 3 to 8; @mod_compare C). However, all models with variation in any or all of these parameters (models 3 to 8) had similar numbers of effective parameters. 


## Model fit to observed counts  
Our expected counts correspond well with the observed counts @fig-spawnercurves and @fig-obsfit. Variation between the observed and expected is quantified in our estimate of spawner count overdispersion ($\mathrm{live}_\phi$). The posterior median of ($\mathrm{live}_\phi$) was `r round(median(post$live_phi), 1)` (95% CI: `r round(quantile(post$live_phi, 0.025), 1)`--`r round(quantile(post$live_phi, 0.975), 1)`).
...
Residence was estimated as `r round(median(post$residence), 1)` days (95% CI; `r round(quantile(post$residence, 0.025), 1)`--`r round(quantile(post$residence, 0.975), 1)`; @fig-residence). 
...
```
Estimated posterior median annual spawner abundances vary from as low as `r round(min(apply(exp(post$log_run), 2, median)))` in `r which(apply(exp(post$log_run), 2, median) == min(apply(exp(post$log_run), 2, median))) + min(spawners$year)-1` to as high as `r round(max(apply(exp(post$log_run), 2, median)))` in `r which(apply(exp(post$log_run), 2, median) == max(apply(exp(post$log_run), 2, median))) + min(spawners$year)-1`. Since 2008, spawner abundance has alternated between low and high levels in a two-year cycle. The posterior estimate from our method is, on average, higher than the AUC estimate ()

in `r sum(AUC_in_66$lower)`% of the `r nrow(AUC_in_66)` years.  

The traditional trapezoidal AUC, with an assumed 11 day residence time, estimate falls within the 66% credible interval of our model prediction in all years. 
