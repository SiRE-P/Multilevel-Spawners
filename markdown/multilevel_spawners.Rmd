...
to estimate spawner abundance. For example, stock assessments of Osoyoos Lake Sockeye are still based on TAUC [@ogden2025] using residence time for Stuart Lake Sockeye estimated from Fofar Creek data in 1946 and 1952 [@killick1955], following the conclusion of Hilborn [@hilborn1999]: “for most data sets where measures of uncertainty are not required, the linear interpolation method is adequate.”
...
\begin{equation}
\boldsymbol{\mu} = [\mu^\text{arrival}, \log(\sigma^\text{arrival} - 1), \log(\sigma^\text{exit} - 1), \log(\mu^\text{exit lag} - 1)].
\end{equation}
...
param_table <- data.frame(
  ...
  Prior = c(
    sprintf("$\\log(N_y) \\sim \\text{Normal}(%.2f, %.2f)$", priors[1,2], priors[1,3]),
    "$\\text{Normal}(\\mu^\\text{arrival}, \\sigma^\\text{arrival})$",
    "$1 + \\exp(\\theta_y)$",
    "$1 + \\exp(\\theta_y)$",
    "$1 + \\exp(\\theta_y)$",
    sprintf("$\\log(\\phi) \\sim \\text{Normal}(%.2f, %.2f)$", priors[7,2], priors[7,3]),
    "$\\text{Normal or MVN}$",
    "Estimated (LKJ prior on correlation)",
    sprintf("$\\mu^\\text{arrival} \\sim \\text{Normal}(%.2f, %.2f)$", priors[2,2], priors[2,3]),
    sprintf("$\\sigma^\\text{arrival} \\sim \\text{Lognormal}(\\log(%.2f), %.2f)$", priors[3,2], priors[3,3]),
    sprintf("$\\mu^{\\text{arrival spread}} \\sim \\text{Normal}(%.2f, %.2f)$", priors[4,2], priors[4,3]),
    sprintf("$\\mu^{\\text{exit spread}} \\sim \\text{Normal}(%.2f, %.2f)$", priors[4,2], priors[4,3]),
    sprintf("$\\mu^{\\text{exit lag}} \\sim \\text{Normal}(\\log(%.2f), %.2f)$", priors[6,2], priors[6,3]),
    sprintf("$\\sigma^{\\text{arrival spread}} \\sim \\text{Lognormal}(\\log(%.2f), %.2f)$", priors[5,2], priors[5,3]),
    sprintf("$\\sigma^{\\text{exit spread}} \\sim \\text{Lognormal}(\\log(%.2f), %.2f)$", priors[5,2], priors[5,3]),
    sprintf("$\\sigma^{\\text{exit lag}} \\sim \\text{Lognormal}(\\log(%.2f), %.2f)$", priors[5,2], priors[5,3])
  )
)
