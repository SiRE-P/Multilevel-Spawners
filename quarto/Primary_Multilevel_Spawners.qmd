...
Spawner abundance is a key variable in salmon ecology. Functions relating spawners to their offspring that return as adults ("stock" and "recruits") are fundamental for management of salmon fisheries and salmon habitats [@ricker1975; @hilborn1992]. For salmon, spawner abundance in a river, stream, or lake beach is typically estimated through repeated visual surveys---for example, from a boat, a helicopter, a drone, or while snorkelling, or walking---that count the number of spawners present across multiple days in the spawning period. However, converting these repeat survey counts into an estimate of total spawner abundance requires adjusting for the fact that spawning fish typically spend multiple days (e.g., 8.8-187 days for sockeye salmon; @perrin1990) on the spawning grounds. 

The conventional method for estimating spawner abundance based on repeat visual surveys calculates the area under the curve (AUC; @ames1977, @perrin1990). This AUC is the integral of the spawner counts over the spawning season and is divided by an assumed residence time (also called survey life)---the average number of days an individual spawner remains in the survey area--- which is estimated independently. Residence times have been found to vary across years and across spawning grounds and so it is recommended that annual, spawning-ground specific estimates should be used because  (@bocking1998; @perrin1990; @irvine1992). There are a range of methods for estimating residence time, but all have limitations (reviewed in @perrin1990). However, because estimating residence time is difficult and requires additional surveying, they are often assumed, sometimes based on similarity between populations (e.g., @hyatt2010). But, because residence time acts as a scalar on the survey counts, it can introduce substantial bias to the resulting total spawner abundance estimates if the estimated value is incorrect.
...
We introduce a new model to estimate **residence** directly and to estimate the precision of **runs** -- these are the key advances of approach 3 over approaches 1 and 2. In addition to the parameter estimates for spawners **arrival timing** (mean day) and **arrival spread** (standard deviation), our model expands and generalizes the concept of **residence** by expanding that parameter into estimates of **exit lag** and **exit spread,** allowing residence to vary by order of arrival. By applying this model to multiple years of spawner surveys simultaneously, as a non-linear, multilevel Bayesian regression, we will be able to determine full use of the survey information while safe-guarding against overfitting.
