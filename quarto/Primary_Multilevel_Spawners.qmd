...
both the amount of interannual variation in each timing parameter and
...
section are walked and fish are enumerated. Since 2012, enumeration of each individual reach within the Index section is available, however, we have used the summed counts across all reaches, because this is the
...
generate our simulated data. Regardless of the gap between surveys, the true value of all parameters falls within the 95% credible interval in at least 95% of annual estimates (@fig-sims A). We find that estimates become less precise as the survey gap increases (@fig-sims B), particularly for exit lag and arrival timing. The model tends to overestimate exit lag (@fig-sims B), but this is not consistent across years or simulations. Otherwise, our estimates of bias suggest that there is no tendency for any of the other parameters to be over or underestimated.
...
($\sigma_y^\text{exit}$) was similar to arrival spread
...
Arrival timing ($\mu_y^\text{arrival}$) and
...
positive relationship (@fig-cor). Arrival spread and exit lag were positively correlated with a posterior median correlation of `r round(median(post$Omega[,2,4]),2)` and
...
negative relationship. Correlations between other combinations of parameters showed weaker support.
...
Our estimated annual abundances are broadly consistent with trapezoidal AUC estimates with an assumed 11 day residence time, but are typically higher: the median across years is `r (round(median(perc_diff$perc_diff), 1))`% higher and the interannual range is from
...
values reliably. The true estimates fall within our 95% credible intervals in at least 95% of the years. The precision decreases as we increase the gaps between surveys, as is expected. But, we find that the model does not tend to be biased in its ability to estimate the parameters, with the exception of exit lag, which is often overestimated. Reassuringly, log run size, which is the primary variable of interest in spawner enumeration surveys, seems to be the most robust parameter to low survey frequency.
...
years, even if the overall distribution is variable. In contrast to the assumption in  @su2001 and @adkison2001 that residence time is a function of order of arrival (early arrivals stay longer), we do not find strong evidence that exit spread tends to be smaller than arrival spread in this system. This pattern has been identified in other studies, where it has been attributed to the fact that salmon of different age and sex
may arrive at different times [@perrin1990; sockeye: @killick1955; pink:
@fukushima1997; @dangel1988] so this difference warrants further study. 
...
CL: Investigation (spawner enumeration surveys), Data curation, Resources.
...
[Zenodo](https://support.zenodo.org/help/en-gb): link to be added once finalized.
