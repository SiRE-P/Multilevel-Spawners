...
index_sk %>% 
  ...
  scale_y_log10()+
  geom_vline(xintercept = 267, lty = 2)

index_sk <- index_sk |> filter(yday > 267)
```
...
priors <- data.frame(prior = c("log_runs_mu", "arrival_mu", "arrival_sigma", "spread","spread_sigma","residence", "count_dispersion"),
                     v1 = c(10,280-day_stand, 2,log(8-1), 1, log(11-1), log(20)), 
                     v2 = c(1,2,0.2,0.2, 0.3, 0.3, 0.5))
...
init_fun <- function() list(
      ...
      arrival_sigma = rlnorm(1, log(1), 0.4), 
...
To account for observation error, a continuous curve (Gaussian (normal)
if symmetric, else beta [@hilborn1999] or Weibull [@labelle2021], as
...
function of arrival date. They also allowed for year-to-year variation
...
start_day <- -5
arrived <- sapply(start_day:max(example_year$day_ind), FUN = function(x) {
...
exited <- sapply(start_day:max(example_year$day_ind), FUN = function(x) {
...
counts <- sapply(start_day:max(example_year$day_ind), FUN = function(x) {
...
ggplot(example_data, aes(x = day + day_stand, y = spawners, group = type))+
  ...
  scale_x_continuous(labels = ~ format(as.Date(.x, origin = "2023-12-31"), "%b ") %>% 
                     ...
                     name = "", expand = c(0, 0))+
  scale_y_continuous(labels = label_number(scale = 1e-3, suffix = "k"), name = "Spawners", expand = expansion(mult = c(0, 0)), limits = c(0, 0.25e5), breaks = seq(0, 0.25e5, by = 0.05e5))+
  geom_point(data = example_year, aes(x = yday, y = live, group = NA))+
...
models follow the processes shown in @fig-enterexit. These models only
deal with live spawners. Spawner counts are integers and can include
...
quantities that are estimated in our model: (1)
$p^\text{arrived}_{y,d}$, the proportion of spawners that have arrived
...
while omitted from fitting (cross-validation), provide diagnostics for
...
*effective residence time* in each year based on the sum of estimated
...
appears in both the numerator and the denominator of @eq-residence, we
...
years of observations, where the annual parameters governing the
spawning timing and run size were drawn randomly from the distributions
in @tbl-sim. These yearly parameters are considered to be the *true*
...
observed data with statistical model 8 (@fig-modcompare), which is the
...
For each year, we quantified precision as the median absolute deviation
(MAD, a robust version of standard deviation) of the posterior draws
around the median and bias as the median of ratios of fitted over true
values. We also quantified coverage as the proportion of years in which
the true value fell within the 95% posterior credible interval.
...
After one to three years (rarely more) of marine growth, adults return
...
Canada (station 08NM085, flow, is also station 08NM0001, water quality).
Map by Braden Judson, Pacific Biological
Station.](../figures/Okanagan_to_Osoyoo_Lakes.png){#fig-map
...
between Osoyoos and Vaseux Lakes (@fig-map) since
...
Outlet Dam (Okanagan Falls Dam in @fig-map). We have excluded these
...
river sections, that vary through time. This extension is a logical next
step, but is not necessary for demonstrating our method, the goal of
this manuscript.
...
Based on ELPD statistics (@fig-modcompare B) we selected model 9 as the
best from those explored. This model was the most complex of those
...
allowed to vary (models 3 to 9; @fig-modcompare C). However, all models
...
generate our simulated data with reasonably low bias, median absolute
deviation, and good 95% posterior credible interval coverage when using
data with a one day gap between surveys (@fig-sims). As the gap between
surveys increases, estimates become more biased, and less precise, but
the degree to which they do so varies by parameter. Our estimate of
total abundance, log run size, is the least sensitive to survey gap;
higher survey gaps lead to a slight negative bias, but this is only
`r round(median(100*(1- sim_summary$ratio[sim_summary$.variable == "log_run" & sim_summary$gap == 7])), 1)`%
with a gap of 7 days between surveys, and the proportion of years for
which the true log run size falls within the 95% posterior credible
estimate only decreases from nearly 100% with 1 and 3 day gaps to \~ 80%
with 5 and 7 day gaps. Arrival timing, arrival spread, and exit spread,
are more sensitive to increasing survey gaps, with their deviations from
the true value increasing and these deviations becoming more negative.
Exit lag is the most sensitive to increased survey gaps; our simulations
show that the model struggles to accurately estimate its true value even
with 1 day gaps, and increasing gaps tend to result in increasing
overestimation.
...
counts (@fig-spawnercurves; @fig-obsfit). Variation between the observed
and expected is quantified in our estimate of spawner count
...
for(j in 1:ncol(post$log_run)){
  live <- sapply(-15:max(sp_dat$day), FUN = function(x) {
    entered <- pnorm(x, post$arrival[,j], post$arrival_spread[,j])
    exited <- pnorm(x, post$arrival[,j] + post$exit_lag[,j], post$exit_spread[,j])
  ...
  spawn_curves.df <- bind_rows(spawn_curves.df, data.frame(
      ...
      day = -15:max(sp_dat$day)+day_stand,
...
Arrival timing ($\mu_y^\text{arrival}$) varied across years
(@fig-timings A), with posterior medians ranging from
`r format(lubridate::ymd("2025-01-01") + round(min(apply(post$arrival, 2, median))+day_stand - 1), "%B %e")`
to
`r format(lubridate::ymd("2025-01-01") + round(max(apply(post$arrival, 2, median))+day_stand - 1), "%B %e")`.
Arrival spread ($\sigma_y^\text{arrival}$) varied across years
(@fig-timings B), with posterior medians ranging from
`r round(min(apply(post$arrival_spread, 2, median)),1)` to
`r round(max(apply(post$arrival_spread, 2, median)),1)` days. Exit lag
($\mu_y^\text{exit lag}$) varied across years (@fig-timings C), with
posterior medians ranging from
`r round(min(apply(post$exit_lag, 2, median)),1)` to
`r round(max(apply(post$exit_lag, 2, median)),1)` days. Exit spread
($\sigma_y^\text{exit}$) tended to be smaller than arrival spread
(@fig-timings D), with posterior medians ranging from
`r round(min(apply(post$exit_spread, 2, median)),1)` to
`r round(max(apply(post$exit_spread, 2, median)),1)` days.

The only timing parameters that showed strong evidence of correlations
in their yearly values were arrival timing ($\mu_y^\text{arrival}$) and
arrival spread ($\sigma_y^\text{arrival}$), with a posterior median
correlation of `r round(median(post$Omega[,1,2]),2)` and
`r round(mean(post$Omega[,1,2]>0)*100)`% of samples supporting a
positive relationship (@fig-cor). There was weaker support for positive
correlations between exit spread ($\sigma_y^\text{exit}$) and arrival
timing (median = `r round(median(post$Omega[,1,3]),2)`; % support =
`r round(mean(post$Omega[,1,3]>0)*100)`) and exit lag (median =
`r round(median(post$Omega[,4,3]),2)`; % support =
`r round(mean(post$Omega[,4,3]>0)*100)`. A negative correlation between
exit spread and arrival spread was also weakly supported (median =
`r round(median(post$Omega[,2,3]),2)`; % support =
`r round(mean(post$Omega[,2,3]<0)*100)`).

Together, the timing parameters result in year-to-year variation in the
derived effective residence time (@fig-timings E). The posterior median
...
`r which(apply(exp(post$log_run), 2, median) == max(apply(exp(post$log_run), 2, median))) + min(spawners$year)-1`.
Since 2008, spawner abundance has alternated between low and high levels
in a two-year cycle.

Our estimated annual abundances are relatively close to those obtained
from the trapezoidal AUC estimate with an assumed 11 day residence time.
The traditional estimates fall within our 66% credible interval in
`r sum(AUC_in_66$in_66)` out of `r length(AUC_in_66$in_66)` years.
Across all years, our model estimates are a median of
`r (round(median(perc_diff$perc_diff), 1))`% higher than those based on
TAUC (inter-annual range:
`r round(min(perc_diff$perc_diff), 1)`--`r round(max(perc_diff$perc_diff), 1)`%).
...
Our simulations demonstrate that the model is able to estimate parameter
values with reasonable precision and low bias, but that is reduced as
the gap between surveys in the dataset increases. This is expected,
since as the amount of data decreases, its relative influence decreases
and the influence of the priors increases. Based on our simulations,
estimates of specific spawner timing parameters, particularly exit-lag,
should be considered less reliable when survey frequency is not high.
However, log run size, which is the primary variable of interest in
spawner enumeration surveys, is relatively robust to low survey
frequency.
...
risk of overfitting to noisy or sparse data. In our application to
Okanagan sockeye, we estimate the median *effective residence* *time* to
be `r round(median(effective_residence), 1)` days, which is close to the
previously assumed 11 days [@hyatt2010], but lower than the 11.9 to 14.1
day estimates based on tilt tags in 2006 and 2007 [@shardlow2011].
However, effective residence time varies across years from
`r round(min(apply(effective_residence, 2, median)), 1)` to
`r round(max(apply(effective_residence, 2, median)), 1)` days,
suggesting that assuming a fixed value introduces bias in some years.
Comparing our annual estimates in 2006 and 2007 to those from
@shardlow2011, we find that our estimates are still lower. In 2006, our
...
Arrival timing was surprisingly consistent, ranging by only
...
days in
`r unique(index_sk$year)[which(apply(post$arrival_spread, 2, median) == min(apply(post$arrival_spread, 2, median)))]`
to `r round(max(apply(post$arrival_spread, 2, median)),1)` days in
...
years, even if the overall distribution is variable. In Okanagan
sockeye, we estimate that exit spread tends to be smaller than arrival
spread, implying that spawner residence time is a function of order of
entry (early arrivals stay longer). This is consistent with the
assumption in @su2001 and @adkison2001, but here we estimate it directly
from the data. This pattern has been identified in other studies, where
it has been attributed to the fact that salmon of different age and sex
may arrive at different times [@perrin1990; sockeye: @killick1955; pink:
@fukushima1997; @dangel1988].
...
processes co-vary. For Okanagan sockeye, we estimate that arrival timing
and arrival spread are positively correlated, indicating that when
arrival is more spread out, this pushes the average arrival date later.
The other parameters are more weakly associated, although there is some
evidence that greater arrival spread (and thus later arrival) is
associated with a more narrow exit spread, which is consistent with
spawners being constrained by biological limits late in the season.
...
that this population might have an unusually high incidence of age x.1
...
abundances, but do not show anomalous spawning timing (@fig-timings).
...
be associated with the Testalinden dam failure; how or if the event
...
negative bias; that is, observers only see a fraction of the fish.
...
fewer annual parameters (@fig-modcompare) might lead to less informative
...
identifiable in @fig-runs is frequently evident [@burgner1991;
...
The data used in this study are the property of the Okanagan Nations
...
[Zenodo](https://support.zenodo.org/help/en-gb):
