...
\mu^\text{live}_{y,d} &= N_y^{\text{spawners}} \times \exp\left( \log\left(1 + 10^4 \times \text{arrived}_{y,d} \times (1 - \text{dead}_{y,d})\right) - \log(10^4) \right) \\
\text{arrived}_{y,d} &= \Phi\left( \frac{ d - \mu^\text{arrival}_y }{ \sigma^\text{arrival}_y } \right) \\
\text{dead}_{y,d} &= \Phi\left( \frac{ d - (\mu^\text{arrival}_y + \mu^\text{residence}_y)}{ \sigma^\text{exit}_y } \right)\\
...
\log(\mu^\text{residence}_y-1) &\sim \text{Normal}(\mu^\text{residence}_\mu, \mu^\text{residence}_\sigma)\\
\mu^\text{residence}_\mu &\sim \text{Normal}(\log(10), 0.3)\\
\mu^\text{residence}_\sigma &\sim \text{Exponential}(10)\\
...
The average exit spread ($e^{\mathrm{exit\_spread}_{\mu}}$) across years was estimated to be `r round(median(exp(post$death_spread_mu)), 1)` days (posterior median; 95% CI: `r round(quantile(exp(post$death_spread_mu), 0.025), 1)`--`r round(quantile(exp(post$death_spread_mu), 0.975), 1)`). Exit spread varied across years by a multiplicative factor ($e^{\mathrm{exit\_spread}_{\sigma}}$) of `r round(median(exp(post$death_spread_sigma)), 1)` As a result, the median estimated exit spread ranged from `r round(min(apply(post$death_spread, 2, median)) - 1)` to `r round(min(apply(post$death_spread, 2, max)) - 1)` days (@fig-timings C).
