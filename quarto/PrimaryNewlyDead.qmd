...
\text{exited}_{y,d} &= \Phi\left( \frac{ d - (\mu^\text{arrival}_y + \mu^\text{exit_lag}_y)}{ \sigma^\text{exit}_y } \right)\\
...
\log(\mu^\text{exit_lag}_y-1) &\sim \text{Normal}(\mu^\text{exit_lag}_\mu, \mu^\text{exit_lag}_\sigma)\\
\mu^\text{exit_lag}_\mu &\sim \text{Normal}(\log(10), 0.3)\\
\mu^\text{exit_lag}_\sigma &\sim \text{Exponential}(10)\\
...
data.frame(model = c("m1","m2","m3","m4", "m5", "m6","m7", "m8"), 
  ...
  labels = c(
    ...
    "exit_spread" = expression(sigma[y]^death),
    "residence" = expression(mu[y]^exit_lag)
  )
)+
...
Exit spread ($\sigma_y^\text{exit}$) tended to be lower than arrival spread (@fig-timings C), with posterior medians ranging from `r round(min(apply(post$death_spread, 2, median)))` to `r round(max(apply(post$death_spread, 2, median)))` days.Exit lag ($\mu^\text{exit_lag}$) was estimated as `r round(median(post$residence), 1)` days (95% CI; `r round(quantile(post$residence, 0.025), 1)`--`r round(quantile(post$residence, 0.975), 1)`). 

Together, the timing parameters result in year-to-year variation in the mean residence time (@fig-timings D). The posterior median across years is `r round(median(effective_residence$effective_residence), 1)` days, but it ranges from `r round(group_by(effective_residence, year) %>% summarise(residence = median(effective_residence)) %>% pull(residence) %>% min(), 1)` to `r round(group_by(effective_residence, year) %>% summarise(residence = median(effective_residence)) %>% pull(residence) %>% max(), 1)` days. 




The previously assumed 11 day residence time corresponds to the `r round(ecdf(post$residence)(11)* 100)`$^th$ percentile of our estimated posterior distribution. In the alternative model (m8) where residence was allowed to vary across years, the median residence time across years was `r round(median(apply(post8$residence,2,median)), 1)` days (@fig-timingsalt D; inter-annual range: `r round(min(apply(post8$residence,2,median)), 1)`--`r round(max(apply(post8$residence,2,median)), 1)`).

```{r fig-timings, echo = FALSE, warning=FALSE, message=FALSE, fig.cap="Posterior distributions of timing, entry_spread, and exit_spread. Points and error bars show the posterior median and the 66% and 95% credible intervals.", fig.align='center', fig.height=9, fig.width=6}
