...
index_sk_trimmed <- index_sk %>% filter(live >0)

sp_dat = list(n_priors = nrow(priors), 
              priors = data.matrix(priors[,-1]),
              n_years = max(index_sk_trimmed$year_ind),
              year = as.numeric(factor(index_sk_trimmed$year)),
              day = index_sk_trimmed$yday-day_stand,
              n_obs = nrow(index_sk_trimmed),
              live_counts = index_sk_trimmed$live)
...
run_or_load_stan <- function(model_name, stan_path, data_list, chains = 4, cores = 4) {
  ...
  if (!file.exists(fit_file) || !file.exists(hash_file) || readLines(hash_file) != current_hash) {
    ...
    fit <- sampling(m, data = data_list, chains = chains, cores = cores, init = replicate(4, list(arrival_mu = rpois(n =1, lambda = 35)), simplify = FALSE))
...
$$N_{y,d}^{\text{live}} \sim \text{NegBinomial}(\mu^\text{live}_{y,d}, \phi^{\text{live}})$$ {#eq-obs}
...
$$\mu^\text{live}_{y,d} \approx p^\text{arrived}_{y,d} \times (1 - p^\text{exited}_{y,d}) \times N_y^{\text{spawners}}.$$ {#eq-live}
...
$$\mu^\text{live}_{y,d} =  \exp\left( \log\left(1 + 10^4 \times p^\text{arrived}_{y,d} \times (1 - p^\text{exited}_{y,d})\right) - \log(10^4) \right) \times N_y^{\text{spawners}}$$ {#eq-livefull}
