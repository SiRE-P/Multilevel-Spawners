...
\mu^\text{live}_{y,d} &= N_y^{\text{spawners}} \times \exp\left( \log\left(1 + 10^4 \times \text{arrived}_{y,d} \times (1 - \text{exited}_{y,d})\right) - \log(10^4) \right) \\
\text{arrived}_{y,d} &= \Phi\left( \frac{ d - \mu^\text{arrival}_y }{ \sigma^\text{arrival}_y } \right) \\
\text{exited}_{y,d} &= \Phi\left( \frac{ d - (\mu^\text{arrival}_y + \mu^\text{residence}_y)}{ \sigma^\text{exit}_y } \right)\\
...
Exit spread ($\sigma_y^\text{exit}$) tended to be lower than arrival spread (@fig-timings C), with posterior medians ranging from `r round(min(apply(post$death_spread, 2, median)))` to `r round(max(apply(post$death_spread, 2, median)))` days.
...
Residence ($\mu^\text{residence}$) was estimated as `r round(median(post$residence), 1)` days (95% CI; `r round(quantile(post$residence, 0.025), 1)`--`r round(quantile(post$residence, 0.975), 1)`; @fig-residence). The previously assumed 11 day residence time corresponds to the `r round(ecdf(post$residence)(11)* 100)`$^th$ percentile of our estimated posterior distribution. In the alternative model (m8) where residence was allowed to vary across years, the median residence time across years was `r round(median(apply(post8$residence,2,median)), 1)` days (@fig-timingsalt D; inter-annual range: `r round(min(apply(post8$residence,2,median)), 1)`--`r round(max(apply(post8$residence,2,median)), 1)`).

```{r fig-residence, echo = FALSE, warning=FALSE, message=FALSE, fig.cap="Posterior distribution of the estimated mean residence time. Points and error bars show the posterior median and the 66% and 95% credible intervals. The vertical dashed line shows the 11 day residence time that has previously been assumed for this system.", fig.align='center', fig.height=4, fig.width=8}
effective_residence <- data.frame()
for(j in 1:ncol(post$log_run)){
  live <- sapply(1:max(sp_dat$day), FUN = function(x) {
    entered <- pnorm(x, post$arrival[,j], post$arrival_spread[,j])
    exited <- pnorm(x, post$arrival[,j] + post$residence, post$death_spread[,j])
  ...
  effective_residence <- bind_rows(effective_residence, data.frame(year = j + min(spawners$year)-1, 
                                                           effective_residence = apply(live, 1, sum)/exp(post$log_run[,j])))
}
...
gather_draws(fit, residence) %>% 
  ...
  ggplot(effective_residence, aes(x = year, y = effective_residence))+
  stat_pointinterval()+
  geom_hline(yintercept = 11, lty = 2)+
  labs(y = "Effective residence time", x = "")
```
